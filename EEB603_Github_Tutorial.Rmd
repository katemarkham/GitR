...
```{r figure1, echo=FALSE, fig.cap="Figure 1. Creating a repository", out.width = '100%'}
...
```{r figure2, echo=FALSE, fig.cap="Figure 2. What to Type in the Command Prompt", out.width = '100%'}
...
After the "GNU General Public License" , keep the default options for the next two screens, then for "Adjusting your PATH environment", select "Use Git from the Windows Command Prompt" (Figure 3),
```{r figure3, echo=FALSE, fig.cap="Figure 3. Adjusting your path environment", out.width = '100%'}
knitr::include_graphics("Git_Install3.PNG")
```
and then continue and keep the remaining defaults.
5. In your Command Prompt, type 
git --version
to check that your install was successful (see Figure 2)
...
1.Type
git clone AND THEN THE HTTPS YOU JUST GRABBED
2. Type
cd test_github
...
```{r figure4, echo=FALSE, fig.cap="Figure 4. Connecting your repository to Git", out.width = '100%'}
...
```{r figure5, echo=FALSE, fig.cap="Figure 5. Sharing your code with RStudio and Git", out.width = '100%'}
knitr::include_graphics("Git_and_RStudio.png")
```
